...
trait RequestParameterValueResolverTrait
{
    ...
    final protected function resolveFromRequestParameters(Request $request, ArgumentMetadata $argument): array
    {
        ...
        } elseif ($originalBag->has($requestParameterName)) {
        ...
        if (null !== $filter && !empty($values)) {
            ...
            if (\FILTER_VALIDATE_BOOL === $filter) {
            ...
            foreach ($values as $i => $value) {
                ...
                if ('' === $value && $isNullOnEmptyString && (\FILTER_VALIDATE_INT === $filter || \FILTER_VALIDATE_FLOAT === $filter)) {
                    unset($values[$i]);
                    continue;
                }
                ...
                if ((false === $filtered && !$isNullOnFailure) || (null === $filtered && $isNullOnFailure)) {
                    $failedValues[] = $value;
                    unset($values[$i]);
                } else {
                    $values[$i] = $filtered;
